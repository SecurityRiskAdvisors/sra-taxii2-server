...
var module, window, define, renderjson=(function() {
    var themetext = function(/* [class, text]+ */) {            //takes pairs of arguments, returns span classes. so themetext("AAA", "BBB", "CCC") -> [<span class="AAA">BBB</span>, <span"CCC"></span>]
        var spans = [];
    ...
    var append = function(/* el, ... */) {                        // seems to append args 1..n to arg 0
        var el = Array.prototype.shift.call(arguments);           //? leading tags
        for (var a=0; a<arguments.length; a++)                    //? iterate through the contents of the tag
    ...
    var text = function(txt) { 
      var start_index = txt.search(/http/);
      if(start_index > -1){
        var end_index = txt.substring(start_index).search(/"/)+start_index;
        var link_text = txt.substring(start_index, end_index);
        var rv = [];
        rv.push(document.createTextNode(txt.substring(0, start_index)));
        rv.push(link(link_text, link_text));
        rv.push(document.createTextNode(txt.substring(end_index)));
        return rv;
      }
      return document.createTextNode(txt);                 // Will probably do link parsing here
    };
    ...
    var A = function A(txt, classname, callback) { var a = document.createElement("a");
                                                  ...
                                                  // a.appendChild(text(txt));
                                                   append(a, text(txt));    //might handle array of textnode and link?
                                                   a.href = '#';
                                                   a.onclick = function(e) { callback(); if (e) e.stopPropagation(); return false; };
                                                   return a; };
    var link = function(txt, dest) { var a = document.createElement("a");
                                      a.appendChild(document.createTextNode(txt));   //doesn't use text() to avoid infinite pingpong
                                      a.href = dest;
                                      return a;
    };
    ...
    renderjson.set_collapse_msg(function(inpt) { if(typeof(inpt)=="number") 
                                                    return inpt + " item" + (inpt==1 ? "" : "s");
                                                  else if(typeof(inpt)=="object"){
                                                    if(inpt.title != undefined)
                                                    {
                                                      return inpt.title;
                                                    }
                                                    else
